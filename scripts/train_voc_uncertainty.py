...
from wetr.model_simple_mit_uncer import WeTr
...
def validate(model=None, data_loader=None, cfg=None):
    ...
    with torch.no_grad():
        for _, data in tqdm(enumerate(data_loader),
            ...
            cls, segs, _, _ = model_without_ddp(inputs,)
            ...
            _cams = multi_scale_cam(model_without_ddp, inputs, cfg.cam.scales)
...
def train(cfg):
    ...
    wetr.to(args.local_rank)
    
    in_channels_for_uc = wetr.in_channels[-1]
    ...
    for n_iter in range(cfg.train.max_iters):
        ...
        cls, segs, feature_map, z = wetr(inputs, seg_detach=args.seg_detach)
        ...
        cams = multi_scale_cam(wetr.module, inputs=inputs, scales=cfg.cam.scales)
        ...
        if (n_iter+1) % cfg.train.eval_iters == 0:
            if dist.get_rank() == 0:
                ...
                torch.save(wetr.module.state_dict(), ckpt_name)
                
                logging.info('Validating...')
                cls_score, seg_score, cam_score = validate(model=wetr, data_loader=val_loader, cfg=cfg)
